...
export type ReactNativeDevtoolsRegistry = ReactNativeSSEDevtoolsRegistration & {
  readonly subscribe: (listener: () => void) => () => void;
  readonly getSnapshot: () => ReactNativeDevtoolsSnapshot;
...
export function createReactNativeDevtoolsRegistry(
  options: { readonly maxEvents?: number } = {}
): ReactNativeDevtoolsRegistry {
  const maxEvents = options.maxEvents ?? MAX_EVENTS;
  const clients = new Map<string, ReactNativeDevtoolsClientRecord>();
  const listeners = new Set<() => void>();
  let snapshot: ReactNativeDevtoolsSnapshot = new Map();
  ...
  function patch(
    id: string,
    update: Partial<Omit<ReactNativeDevtoolsClientRecord, "id" | "client">>
  ): void {
    ...
    const hasChanges = Object.entries(update).some(([key, value]) => {
      ...
      return record[key as keyof ReactNativeDevtoolsClientRecord] !== value;
    });
  ...
  return {
    register(info: ReactNativeSSEDevtoolsClientInfo): () => void {
      ...
      const unsubEvents = client.subscribeAnyEvent((event) => {
        ...
        const entry: ReactNativeDevtoolsEventEntry = {
    ...
    getSnapshot(): ReactNativeDevtoolsSnapshot {
      if (snapshotDirty) {
