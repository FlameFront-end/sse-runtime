...
| `reconnect`         | `(options?: { reason?: string; timeout?: number }) => Promise<void>`                                           | Force a fresh shared connection even when the client reports `open`, resuming from the last seen event id. In single-tab coordination, a follower requests reconnect from the leader and waits for acknowledgement; concurrent reconnect calls are coalesced.                        |
| `ensureOpen`        | `(options?: { timeout?: number }) => Promise<boolean>`                                                         | Wait until the connection status is open. Starts connecting if needed. Resolves `true` when open, `false` on terminal failure, rejects if the optional timeout expires. It returns immediately for an already `open` client and does not prove the stream is still delivering bytes. |
| `ensureHealthy`     | `(options: { staleAfter: number; timeout?: number; reason?: string }) => Promise<boolean>`                     | Return `true` only when status is `open` and byte-level transport activity is fresh. If activity is stale or missing, force a fresh shared transport and wait for it to open. Use this before sending commands whose response arrives over SSE.                                      |
| `getStatus`         | `() => SSEConnectionStatus`                                                                                    | Synchronous status snapshot.                                                                                                                                                                                                                                                         |
| `getError`          | `() => SSEError \| null`                                                                                       | Last error, or null.                                                                                                                                                                                                                                                                 |
| `getLastActivityAt` | `() => number \| undefined`                                                                                    | Timestamp (`Date.now()`) of the latest accepted transport or received chunk, including comment heartbeats. Preferred for liveness checks.                                                                                                                                            |
| `getLastEventAt`    | `() => number \| undefined`                                                                                    | Timestamp (`Date.now()`) of the most recently parsed application event, or `undefined` if none yet. Useful for app-event diagnostics, not transport liveness.                                                                                                                        |
| `getLastRecovery`   | `() => SSERecoveryEvent \| undefined`                                                                          | Latest recovery lifecycle event: `{ phase, reason, timestamp }`, where `phase` is `"requested"`, `"started"`, `"succeeded"`, or `"failed"`.                                                                                                                                          |
...
| `subscribeActivity` | `(listener: (timestamp: number) => void) => () => void`                                                        | Subscribe to byte-level transport activity. Fires immediately if an activity timestamp already exists.                                                                                                                                                                               |
| `subscribeRecovery` | `(listener: (event: SSERecoveryEvent) => void) => () => void`                                                  | Subscribe to recovery lifecycle events. Fires immediately if a recovery event already exists.                                                                                                                                                                                        |
...
const detach = attachLifecycleResume(client, {
  ...
  staleTimeoutMs: 120000, // treat an open stream with no bytes for 2 min as dead
...
| `staleTimeoutMs`    | `number`                                             | —                          | When set, an open stream with no byte-level activity for this long counts as stale: a watchdog reconnects it, and unforced triggers (`focus`, `pageshow`) only resume when the connection looks unhealthy. |
...
| `getLastActivityAt` | `() => number \| undefined`                          | `client.getLastActivityAt` | Source for the transport-activity timestamp used by staleness checks.                                                                                                                                      |
...
```ts
const { status, error, role, connect, disconnect, reconnect, ensureOpen, ensureHealthy, client } =
  useSSE<Events>(options);
```
...
| `reconnect`     | `(options?: { reason?: string; timeout?: number }) => Promise<void>`                       | Force a fresh shared connection (see `SSEClient.reconnect`)                      |
| `ensureOpen`    | `(options?: { timeout?: number }) => Promise<boolean>`                                     | Wait until status is open; starts connecting if needed                           |
| `ensureHealthy` | `(options: { staleAfter: number; timeout?: number; reason?: string }) => Promise<boolean>` | Wait until the shared transport is open and recently active                      |
